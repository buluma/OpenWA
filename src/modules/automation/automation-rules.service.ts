...
/** Structural shape of the one MessageService method this service calls, resolved lazily below. */
interface TextSender {
  sendText(sessionId: string, dto: { chatId: string; text: string }): Promise<unknown>;
}
 ...
 *
 * The reply is sent through `MessageService.sendText`, resolved lazily via `ModuleRef` rather than
 * injected in the constructor: `MessageModule` imports `SessionModule`, and this module is imported
 * BY `SessionModule` (the projector calls `evaluateInbound`), so a constructor dependency on
 * `MessageService` here would close a module cycle. `ModuleRef.get(..., { strict: false })` resolves
 * it lazily at first use instead. In unit contexts without a `ModuleRef` the evaluator logs and skips.
 */
@Injectable()
export class AutomationRulesService {
  ...
  async evaluateInbound(sessionId: string, message: Record<string, unknown>): Promise<void> {
    ...
    try {
      const textSender = this.resolveTextSender();
      if (!textSender) return;
      await textSender.sendText(sessionId, { chatId, text: rule.replyText });
  ...
  private resolveTextSender(): TextSender | undefined {
    if (!this.textSender) {
      try {
        // Lazy require avoids a static import of MessageService, which would close the file cycle
        // session (projector) -> automation -> message -> session. By reply time every module is
        // loaded, so resolving the class through ModuleRef at first use sidesteps the cycle entirely.
        // Same technique as PluginHostServices.getMessageService() (src/core/plugins/plugin-host-services.ts).
        const mod =
          // eslint-disable-next-line @typescript-eslint/no-require-imports
          require('../message/message.service') as typeof import('../message/message.service');
        this.textSender = this.moduleRef?.get(mod.MessageService, { strict: false });
      } catch (error) {
